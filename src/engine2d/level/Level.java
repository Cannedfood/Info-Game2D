...
public class Level extends GameMath {
    ...
    /** Gets a tile below a specific coordinate. If you don't know if it will be out of bounds use probeTile(x,y) instead.
    ...
    /** Gets a tile at a specific coordinate. If you don't know if it will be out of bounds use probeTile(x,y) instead.
     * @param x The x coordinate of the Tile
     * @param y The y coordinate of the Tile
     * @return The tiles at { x, y } or null.
     */
    ...
    public Tile probeTile(float x, float y) { return probeTile((int)x, (int)y); }
    
    /** Gets a tile at a specific coordinate, or null if it is out of bounds. */
    public Tile probeTile(int x, int y) {
        if(x < 0 || x >= mWidth) {
            //System.out.println("xnull : " + x + " (width = " + mWidth + ")");
            return null;
        }
        if(y < 0 || y >= mHeight) {
            //System.out.println("ynull : " + y + " (height = " + mHeight + ")");
            return null;
        }
        return getTile(x, y);
    }
    ...
    public void emitTileUpdate(int x, int y) {
        ...
        for(int xx = x - 1; xx <= x + 1; xx++) {
            for(int yy = y - 1; yy <= y + 1; yy++) {
                if(xx != x || yy != y) {
                    Tile t = probeTile(xx, yy);
                    if(t != null) t.onTileUpdate(xx, yy);
                }
            }
        }
    }
